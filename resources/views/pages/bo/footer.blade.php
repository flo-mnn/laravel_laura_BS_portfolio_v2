...
@section('content')
    ...
    <section id="editFooter" class="container">
        <div class="mt-5">
            <h1 class="bo"style="color: {{$color->color}}">Footer Infos</h1>
            <table class="table table-striped table-dark">
                ...
                <tr>
                    ...
                    <td>{{$footers->link}}</td>
                    <td><a href="{{route('footers.edit',['footer' => $footers])}}" style="background-color: {{$color->color}}" class="btn rounded-0 text-light">edit</a></td>
                </tr>
                </tbody>
            </table>
        </div>
        <div class="mt-5">
            <h1 class="bo"style="color: {{$color->color}}">Social Media Items</h1>
            <table class="table table-striped table-dark">
                <thead class="thead-{{$color->color}}">
                <tr>
                    <th scope="col">#</th>
                    ...
                    <th colspan="4">
                        <form class="p-5 rounded-0 bg-dark text-light" action="/socials" method="POST">
                            ...
                            <div class="form-group">
                              ...
                              <input type="text" class="form-control rounded-0 bg-light" style="color: {{$color->color}}" id="icon" name="icon" value="{{old('icon') ? old('icon') : $social->icon}}" aria-describedby="iconHelp">
                            ...
                            <div class="form-group">
                              ...
                              <input type="url" class="form-control rounded-0 bg-light" style="color: {{$color->color}}" id="url" name="url" value="{{old('url') ? old('url') : $social->url}}">
                            </div>
